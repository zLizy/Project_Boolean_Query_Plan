...
def inference():
    # preprocess start
    addition = '_ap_cnf_accu_1'
    if not os.path.exists('output/tables/coco'+addition):
        os.makedirs('output/tables/coco'+addition)
    ...
    path = 'output/tables/coco'+addition+'/'+args.base+'.csv'
    ...
    # Load time
    path_time = 'output/tables/coco'+addition+'/time_'+approach+'.csv'
    ...
    query_parameters = pd.read_csv('repository/model_config_task_half_all.csv', index_col='index').loc[(args.experiment_index)]
    ...
    df_time.to_csv('output/tables/coco'+addition+'/time_'+approach+'.csv')
